...
public:
    ...
    explicit Private(Chip8EmulatorHost& host, Cdp1802Bus& bus, const Chip8EmulatorOptions& options) : _host(host), _cpu(bus), _video(options.behaviorBase == Chip8EmulatorOptions::eCHIP8VIP_8X ? Cdp186x::eVP590 : Cdp186x::eCDP1861, _cpu, options) {}
...
static std::map<std::string,PatchSet> g_patchSets = {
    ...
    },
    {
        ...
        {{
            ...
            {0x084, {0x30, 0xe0}},
            {0x0d7, {0x30, 0xe6}},
            {0x0e1, {0x7c, 0x00, 0xbc, 0x30, 0x86, 0x9c, 0x7c, 0x00, 0xbc, 0xfb, 0x10, 0x30, 0xfc}},
    ...
    },
    {
        ...
        {{
            ...
            {0x20b, {0x45, 0xfa, 0x0f, 0xaf, 0x32, 0x31, 0x46, 0xfa, 0x3f, 0xf6, 0xf6, 0xf6, 0x22, 0x52, 0xe2, 0x06, 0xfa, 0x1f, 0xfe, 0xfe, 0xfe, 0xf1, 0xac, 0x12, 0xf8, 0xd0, 0xbc, 0x07, 0x5c, 0x9c, 0xfc, 0x08, 0xac, 0x2f, 0x8f, 0x3a, 0x26, 0xd4, 0x07,
...
uint8_t Chip8VIP::readByteDMA(uint16_t addr) const
{
    if(addr < 0x1000)
        return _impl->_ram[addr];
    if(addr >= 0xC000 && addr < 0xD100)
        return _impl->_colorRam[addr & 0xff];
    if(addr >= 0x8000 && addr < 0x8200)
        return _impl->_rom[addr & 0x1ff];
    return 0;
}
